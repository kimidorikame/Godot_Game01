...
- `BASE_PRICE = 80` … ベース代。食肉仲卸の支払い額と、クズ野菜への切り替え
  しきい値の両方に使う
- `FINAL_DAY = 3` … 体験版の最終日（本番=7日版に戻すときはここを直接7にする）
...
MARKET（options=7店舗）… ここで止まる（WAITING_INPUT）
  食肉仲卸 … 常に「訪問済み」表示（自動で済んでいる）。クズ野菜の日は
    「食肉仲卸」と「端材半端物（訪問済み）」の表示がまるごと入れ替わる
...
- **お金が無い日のベース代用（クズ野菜）は実装済み**（§6「クズ野菜ベース」参照）。
  水道代・場所代が払えないときの**ゲームオーバー**はまだ未実装（次に着手予定）。
  払えるかのチェック自体、今の`PAY`イベントには無い
- **終了日は実装済み**（§6「終了日」参照）：体験版は`FINAL_DAY`=3日で終了、
...
[済] 終了日（FINAL_DAY=3。最終日CLOSEに「（終了）」テキスト、NEXT_DAYで
     reset_for_new_game()により状態変数10個すべてを初期化して1日目に戻る）
```
...
**ゲームを閉じるための仕様も検討済み**（体験版として3日版でリリースする方針から
逆算）。クズ野菜ベース・終了日は実装済み、残るはゲームオーバーのみ：
- **ゲームオーバー（未実装・次はこちら）**：水道代・場所代が払えない
  （`PAY`直前に所持金が足りない）ときに発生。「（ゲームオーバー）」テキストを
  出して止める、が最小実装の想定。ベース代（食肉仲卸）はクズ野菜で必ず
  賄えるのでゲームオーバーの対象外

**次の候補**（順序は未定。今回の申し送り：クズ野菜→終了日→ゲームオーバー
の順で着手。あと1つ）：
- ゲームオーバー（上記。次はこちら）
