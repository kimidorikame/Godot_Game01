...
**（実装済み・シェルのみ）常時アクセスできる本物のスマホUIを追加した**：
`[スマホ]`ボタンでいつでも（WAKE・PREP・OPEN、鍋モード・市場・ADJUST中を問わず）
開閉でき、開いている間は進行を完全に止める。レシピ／ニュース／SNSの3タブは
現時点ではプレースホルダーで、この節が想定する「起床後に読むニュース・DM」の
実データはまだ載っていない。WAKEの「スマホを見る。」という一言（`WAIT_INPUT`）は
今回のスマホUIとは別の独立した仕組みのまま残してあり、両者をどう繋げる
（このシェルに置き換える／並存させる）かは、Day3以降の日別コンテンツを
書きながら別途決める。詳細はCURRENT_SPEC.md §5「スマホUI（シェルのみ・実装済み）」
参照。
...
Soup（目指す姿）
  base_tags[]  … 出汁の方向性（共通）
  残量         … 客に出すと減る。水を足すと増える（ベースは増やさない。実装済み・下記参照）
...
- **量的劣化**：客が来る→残量が減る→水を足す（かさ増し）→薄まる→ベースを足す→味が戻る
  （実装済み：量を戻せるのは水だけ。ベースは味＝濃さだけを戻す。下記「状態の変化」参照）
...
STEP 17.6 で夜の核（会話→3枠→段階評価）が動いたので、7.5 の残りを具体化した。
**鍋の管理とモブ客はセットで意味を持つ**（客が増えても鍋が無限なら緊張が生まれない）。

### 鍋の役割：好みを当てる装置ではなく、量と品質を管理する装置

企画冒頭には「万人向けの無難な鍋か、特定客に刺さる尖った鍋かの賭け」という
構想があったが、**鍋の tags は評価に数えない**と決めた（STEP 17.6）。理由：

- 鍋タグを得点にすると、3枠を使わずに全客へ**無料の一致点を配る**ことになり、
  会話からトッピングを推測する面白さが弱まる。
- したがって 7日版では **ベース（鶏骨＋手羽端）の一種類に固定**し、
  鍋のゲーム性は **残量と濃さの管理だけ**にする。鍋の種類を選ぶ賭けは扱わない。

### 連鎖の骨格

```
料理の出来（GOOD/GREAT）→ 評判が上がる → 翌日のモブ客が増える
  → 鍋の残量が減る → 水を足す（安い）→ 薄まる → 評価が下がる
  → ベースを足して戻す（金がかかる）
```

**成功が次の困難を生む**構造。制約は2つ：**足せる水は有限**、**ベースは金がかかる**。
この2つがあるから「客が増える＝単純に得」にならない。
...
Soup
  ...
  strength             # 濃さ 1〜5
  water_doses          # 今夜使える水の回数
  reserve_base_units   # 追加用ベースの単位数
```
...
| 2 | 薄め | 影響なし・反応文のみ変化 |
| 3 | ちょうどいい | 影響なし |
| 4 | 煮詰まり気味 | 影響なし・反応文のみ変化 |
| 5 | 煮詰まりすぎ | 最終評価を1段階下げる |

- **鍋の状態で評価を上げることはしない**。悪い状態だけが足を引っ張る。
  `GREAT→GOOD / GOOD→OK / OK→BAD / BAD→BAD`
- 2 と 4 は失敗ではないが、「次に何をすると危ないか」を示す**予兆**になる。
...
**（実装済み・変更点）ベースは残量を増やさない**。当初案では水と同じく＋2として
いたが、「量的劣化は水で戻す／質的劣化はベースで戻す」という上の役割分担と
矛盾していたため、ベースは濃さ＋1だけに直した（`reserve_base_units`の消費は
変わらず−2単位）。**残量を増やせる操作は水だけ**になったことで、鍋が
「補充できるか」を判定するときも水の有無だけを見ればよくなった。

**時間で量は減らない（蒸発は入れない）**。一度は「時間帯が進む → 残量−1」も
入れる案があったが、量は客に出す分で足りる／時間でも減ると管理が増えるという理由で
削った。時間で動くのは濃さだけ。物理的には水分が飛べば量も減るが、
ゲームとしては2つの数値を別の理由で動かす方が扱いやすい。

この関係だから、2つの数値を別々に持つ意味がある：

- 濃い鍋なら **水が安価な正解**
- ちょうどいい鍋を水で増やし続けると**薄くなる**
- 薄い鍋には**追加ベースが必要**（濃さを戻すだけで、量は戻らない。実装済み）
- **残量を増やせるのは水だけ**。ベースは味（濃さ）を戻す専用で、量の不足はベースでは
  埋められない（実装済み）

### ベースと水の扱い

**価格・原価の詳細は別ファイル `PRICING_SPEC.md` を正とする**（売価、仕入れ価格、
定期費、Day1の現金推移、一杯原価の試算など）。ここには鍋の仕組みに関わる部分だけ書く。

**ベース1袋＝80＝10杯分**。内部的には10単位に分ける。

- 朝の仕込み：10単位を使って10杯分の鍋を作る
- 追加ベース：2単位で **濃さ＋1のみ**（残量は変わらない。実装済み・上記「状態の変化」参照）
- 余った未使用単位：**翌日へ持ち越す**
- 完成した鍋の残り：閉店時に廃棄（仕込んだスープは傷む）

単位に分けることで「11杯目のために80を丸ごと失う」価格の崖が無くなる。

**水**は毎朝自動的に2回分を汲む（水代50は週の水場利用料）。翌日へ持ち越さない。
無料で最大量を汲めるなら「何杯汲むか」は常に最大が正解になるので、
**偽の選択肢にしない**。

### 鍋が尽きたとき（実装済み・当初案から変更）

当初は「薄めて出す（売上は入るが評価が下がる）か、断るか」の選択にする案だったが、
実装では**薄めて出す**の道は無くした。残量が客の注文数に届かないまま提供する
ことは無い：**残量が足りるまで水で補充してから提供する**か、水が無くて補充
できなければ**閉店する**かのどちらかになる（ベースは残量を増やせないので、
補充できるかどうかは水の有無だけで決まる）。CURRENT_SPEC.md §5
「鍋が尽きたときの対応」参照。

### 鍋を触るタイミング

**注文後・鍋から取り分ける前**に限定する（「いつでも」ではない）。

```
注文を聞く → 残量確認 → 足りなければ水を足す（補充できなければ閉店）
  → 鍋から取り分ける → 3枠のトッピング → 提供
```

実装では、この「残量確認」は3枠を選んでいる間（ADJUST中）も生きている：
足りないまま提供・廃棄はできず、その場で `[鍋を見る]` を開いて水を足すか、
足せなければ `[閉店]` するかを選ぶ（CURRENT_SPEC.md §5参照）。
...
- 名前あり客は毎日3人で固定＝**物語の量は変わらない**。増えるのはモブだけなので
  シナリオを書く負担が増えない。
- モブの総数は**評判で決まる**。評判が上がるほど鍋の配分が厳しくなる。
...
モブ客（例：港湾労働者の一団 4人）
  ...
  会話        : 一言の要望のみ
				（「荷揚げで疲れた、腹に溜まるやつを」→ FILLING or POWER）

処理:
  一団をまとめて1杯作る → 判定も1回
  → 鍋 −人数分 / 具材 −人数分 / 売上 +50×人数
  → 評判に響く（増加量は名前あり客より小さい）
```

**名前あり客と同じ判定処理を通す**。これが仕様上いちばん重要な点：

- 「モブは評価しない」にすると、「具なしで出せば原価0で儲かる」抜け道ができ、
  それを塞ぐために「調味料と具材を最低1つずつ入れる」という**特別処理**が要る。
- モブも同じ判定にすれば、具なしは自動的に最低評価になり、特別処理が消える。
  判定のコードも1本で済む（名前あり客とモブで分岐しない）。
- 一団は同じ好みを持つ＝同じ仕事帰りの集団なので自然。

**favorite を持たない**理由：個人の好物は「その人を知っているから分かる」もの。
一見の集団には無い。結果として **GREAT は名前あり客だけの報酬**になり、
「その人を知っていることの報酬」という位置づけが保たれる。

**操作は軽いが作業ではない**：4人でも調理は1回なので会話の主役（名前あり客）に
時間を使える。一方で一言の要望から味と具を推測する判断は残る。

**名前あり客の複数杯は、1杯ずつ出す（実装済み・Day1の配達員）**：モブの「一団で1回」と
違い、同じ人が何杯も頼む場合（おかわり・持ち帰り）は、会話の合間に1杯ずつ
調理・提供する。杯ごとに新しい椀を作り、本人の分は1杯ずつ評価して反応を変える。
持ち帰り（本人が食べない杯）は評価しない。評判は客1人分として退店時に1回だけ
反映する（杯数で評判の影響が増えないように）。詳細は CURRENT_SPEC.md §5
「1杯ずつ接客する客」。

**客の並びは日によって変えられる（実装済み・Day2の新人警官）**：`customer_schedule()`
が呼ばれるたびに `day_count` を直接読み、その日だけ別の時間帯構成を返せる
（他の日は今までの戻り値のまま）。専用のState・データ駆動の仕組みは作らず、
関数内の分岐を1つ足すだけで足りた。時間帯（宵の口・夜半・明け方）を客なし
（空配列）にもでき、`OpenController` はその時間帯を読み飛ばして自然にOPENを
終える。詳細は CURRENT_SPEC.md §5「Day2の客：新人警官」。

### 評判のインフレについて

モブも評価されると評判が上がりやすくなるが、**触ってから調整する**。
調整の手段：増加量を小さくする／上限を設ける／BADや品切れで下がる／
評判が高いほど上がりにくくする（逓減）。

なお **鍋が実質的な上限として働く**（10杯分しか無ければ11人目には水かベースが要る）。
評判が上がりすぎても「さばききれない」形で頭打ちになるので、
インフレしても即座に破綻はしない。

### モブ客の未確定

種類（港湾労働者以外に何がいるか。日や時間帯で変わるか）/
評判の増加量（名前あり客の何割か）/ 評判から人数を出す式


### Day1 の構成（数値は触ってから調整する）

```
仕込み : 残量10 / 濃さ3
		 ※ベースは2袋買う。1袋目で仕込み、2袋目から2単位を追加用に使う
		   （残り8単位は翌日へ持ち越し）

宵の口 :
  配達員 3杯（おかわり＋持ち帰り。実装では会話の合間に1杯ずつ出す）→ 残量7
	※「二十往復目」で腹が減っている描写と量が結びつく
  港湾労働者 4杯 → 残量3
  → 尽きかける
  【イベント①】水を足す(+2) → 残量5 / 濃さ2（薄まる）
			  ベースを足す(濃さのみ+1) → 残量5 / 濃さ3（戻る。残量は変わらない）

時間帯移行 : 濃さ4（量は変わらない）

夜半 :
  モブ4人 + チンピラ1杯 → 残量0（ちょうど尽きる）

時間帯移行 : 濃さ5

明け方 :
  【イベント②】老婆「煮立ってるじゃないか、水を足しな」
  → 鍋が尽きているので、老婆に出す前に水を足す(+2) → 残量2 / 濃さ4
	（今夜の水2回分をここで使い切る）
  老婆 1杯 → 残量1
...
**（実装済み・当初案から変わった点）** ベースが残量を増やさなくなったぶん、
以前より余裕が少ない（閉店時の残量が3→1）。水2回分をちょうど使い切る配分に
なり、「今日を取るか、明日を取るか」の緊張は保たれるが、Day1のバランスとして
これでよいかは触ってから判断する（数値は仮のまま）。

- 前半で「薄まる」、後半で「濃くなる」を体験する。**同じ「水を足す」操作が
  状況によって逆の意味を持つ**ことが、1日で分かる。
- 濃さの警告を**システムのアラートではなく客のセリフで**出す。しかも
  「水で薄めた貧乏臭いのはごめんだよ」と言った老婆が「煮立ってる、水を足しな」と言う。
  薄いのも濃いのもダメだと、同じ人物が教える。
- 配達員が3杯食べたことが原因で足りなくなる＝**因果が見える**。

### 画面構成（本番UI の方針）

```
常時表示 : 客 ／ 鍋（残量ゲージ＋数字、濃さ）
切り替え : 客が注文した時点で、鍋の領域が椀（具材＆調味料）の画面に変わる
		   （鍋の上に椀を重ねて表示する案も。後日検討）
```

- 屋台の主人から見た景色の再現。目の前に客、手元に鍋。
- **残量はゲージと数字の両方**。ゲージで「減ってきた」と気づき、数字で
  「あと3杯、夜半は5人来る」と判断する。椀の画面に切り替わっても残量は見える。

### 実装時の警告（重要）

**濃さ管理が毎晩「明け方に水を一回足す」だけの定型作業になる可能性がある。**
モブの人数や来る時間帯が変わっても同じ操作になるなら、濃さは削除して
残量だけに戻すべき。まずこの最小仕様で2〜3日分を動かし、**判断が発生するか**を
確認するところで設計を止める。

### 未確定

断ったときに評判が下がるか / 市場での買い物の操作 / 在庫の消費と腐敗
...
PREP の自動購入（ベース1袋だけ）を、選んで買う市場に広げる。
在庫の穴（具材が無限に使える・予備ベースが初期値固定）を埋める前提。

### 市場の構成（6区画）

同じ街の中にある、という統一感を出すため、区画名（機能的）＋屋号（古びた漢字系）
で揃える。派手な名称にせず「永順」「南興」「福生」「海隆」「張記」くらいの
普通の古い商号に留める。

| 区画名 ― 屋号 | 扱うもの |
|---|---|
| 第一食肉棟 ― 張記肉類加工 | ベース（鶏骨＋手羽端）、モツ、軟骨、すじ肉、切り落とし |
| 青果第三通路 ― 永順青果 | 冬瓜、苦瓜、規格外野菜 |
| 乾貨棟 ― 南興乾貨行 | ナムプリックパオ、ココナッツミルク、薬膳ナンプラーだれ、塩漬けライム、乾燥きくらげ |
| 加工食品路地 ― 福生豆腐麺房 | 豆腐、米麺、生麺、皮類 |
| 水産西棟 ― 海隆鮮貨 | 海老、貝、魚、魚の浮き袋など |
| 荷捌き裏通り ― 半端市「拾味」／共同水場 | 割れた餃子皮、くず肉団子、規格外品、余り物／水場 |

- **張記肉類加工**：Day1「いつもの二袋を取っておいた」が自然に成立する店。
  老張との長い付き合いで、正規品だけでなく骨や端材も回してくれる関係。
  Day1台本の老張のDM（「鶏骨と手羽端、取っといた。遅れるなよ」）と一致する。
- **半端市「拾味」**：端材・半端物だけは正式な屋号にしない。
  「正規の競り・仕分けが終わった後、売れ残り・規格外・破損品・半端なロットが
  流れてくる一角」という通称の店。「割れ皮、今日は二箱あるよ」
  「団子は形悪いだけだ。味は同じ」といった商売。貧しさを恥ではなく
  生活の知恵として描く（企画の核「貧困だが尊厳がある世界」に合わせる）。
- **水産西棟**：TREAT（海老・貝など高級具材）の置き場所。海産物という
  独立したジャンルとして別区画にすることで、「高級だから特別扱い」ではなく
  自然に収まる。
- **共同水場を荷捌き裏通りに置く**：正面の商売空間ではなく裏方設備、という
  位置づけ。「買い物を済ませてから水を汲み、屋台へ戻る」という導線が地理として
  説明される。Day1の水道代イベント（ポンプ番が場所代のように要求してくる）とも
  噛み合う位置づけ。

### 市場の操作フロー

```
市場に入る → 食肉仲卸・加工場（固定・必ず立ち寄る。ベースは毎日必須）
  ↓
[青果][乾物・調味料][豆腐・麺][端材・半端物][海鮮][水場][市場を出る]
  ↓
好きな店を選んで訪れる → 買う（または見るだけ）→ 市場の画面に戻る
  （同じ店に何度でも戻れる。全店を回る必要はない）
  ↓
[市場を出る] を選ぶ（プレイヤーが自分で決める）
  → 水場に寄っていなければ自動で寄る（水を汲み忘れて詰むことを防ぐ）
  → 仕込みへ
```

- **Day1 は水場と「市場を出る」以外は選べない**（食肉仲卸は固定で自動）。
  他の区画は2日目から解禁。
- 各店の訪問は選択式で、押した店だけ訪れる。強制的な周回はない。
...
**ベースは「骨と大根」から「鶏骨＋手羽端」に変更**。大根を市場のどこにも
置く必要が無くなる。手羽端は正規の部位ではなく端材＝ベースそのものが
端材で構成される。企画の核（貧しさが材料に表れる）を土台の部分で体現する。
※ Day1 台本（day1_events.gd の「骨と大根を受け取った」「骨の出汁が立ってきた」）、
6章・7.7の老張DM引用、7.6のベース表記、GENTLE具材例（大根）は対応済み。

**苦瓜は「味だけ持つ具材」**。他の4つの味素材（ナムプリックパオ等）が
加工された調味料なのに対し、苦瓜だけ生野菜そのもの。仕入先は永順（青果）だが、
役割・データ構造上は他の調味料と同格（Ingredient として tags=["BITTER"] を
持つだけで、具タグは持たない）。現実の分類（生野菜）とゲーム上の役割
（味素材）は別、という考え方の実例。

**張記には高級な肉加工品も世界観上は存在するが、実装しない**。
プレイヤーがどうせ買わない（拾味の安い版で十分）ものを、わざわざ
データとして作る必要はない。「有るが、作らない」の判断。

**香草は今回省く**。挙がっていたが tags 未定のまま保留。

**同じ具の軸が複数店にまたがるのは意図どおり**（例：POWER は張記のモツと
拾味のくず肉団子）。正規の店で相応の値段を払うか、端材で安く済ませるかの選択。

### 価格帯（軸とは別の横串。数値は仮）

| 価格帯 | 一杯あたり原価目安 | 例 |
|---|---:|---|
| 廃棄寸前・端材 | 1〜2 | 規格外野菜、拾味の商品 |
| 安い野菜 | 2〜3 | 冬瓜、苦瓜、白菜 |
| 豆腐・卵・海藻 | 3〜5 | 豆腐、卵、海藻 |
| 安い肉 | 5〜7 | 鶏、ひき肉 |
| 肉・軟骨 | 7〜9 | 豚軟骨、くず肉団子 |
| 高級具 | 10〜15 | 魚の浮き袋、キヌガサタケ等 |

具体的な数値は7日間動かしてバランス調整の段階で決める（PRICING_SPEC 11章の方針と同じ）。
店ごとに扱う価格帯におおよその傾向がある（拾味・永順は安い帯、海隆は高級帯）。

### 未確定

在庫の持ち方：**数量を持つ辞書に変える方向**（`{ "tofu": 3, "coconut_milk": 1 }` 等）。
今の `inventory`（文字列の配列、1個=1要素）では市場での複数購入を表しにくいため。
既存の `add_inventory` / `remove_inventory`（唯一の入口）の中身を変える形になる見込み。
実装時に具体案を提示してもらう。

買う操作の具体的なUI（選択肢ボタンの流用か、別の形か）
...
**作り直すときの指針（3枠実装で分かったこと）**：選択肢を1列に全部並べる形は、
種類が増えると探しにくくなる（7つなら問題ないが、調味料も具材も増える予定）。
**調味料と具材は表示を分ける**（別ウィンドウ／別の枠など）。両者は性質が違い、
選ぶときの判断も別（味を決める／体の要求に応える）ため。本番UIでは
「調味料は棚、具材は鍋の脇」のような物理的な配置として表現できる。
...
**枠を増やすときの必須課題（→ STEP 17.6 で解決済み）**：現行の「wanted_tag が含まれれば
GOOD」のままで枠を増やすと、`まろやか＋辛い＋酸っぱい＋苦い` を全部入れれば**どの客にも
必ず当たる**。この穴は STEP 17.6 の設計（3枠／具材は別軸／具0は最低評価／
味の組み合わせ登録）で塞いだ。実装時はそちらを参照。
...
### STEP 17.6：3枠・具材の軸・段階評価（実装済み）

3段階に分けて実装した：
1. 具材を追加し、3枠で選べるようにする（「選ぶ」と「進む」を分離）
2. 判定を一致数による段階評価にする（BAD / OK / GOOD）
3. favorite（好物）でクリティカル（GREAT）を足す

STEP 17.5 の検証で「会話から味を推測する」が面白いと確認できたら、次の順で広げる。

**基本の流れ（目指す形）**：
```
レシピを見る（ヒント） → 3枠に味と具を選ぶ（配分は自由） → 提供
```

#### 枠は3つ。味と具が同じ枠を取り合う

- **椀に足せるのは3つ**。4枠案から3枠へ縮小した（操作が軽い／7日版には3枠で足りる／
  4つ以上はトッピングとして盛りすぎで高級感が出てしまう）。拡張は後日必要になってから。
- 配分は自由：**味1＋具2** か **味2＋具1**。
- **味3つ＝具0 は不可**（具なしの椀は常に最低評価。金を取って具なしは論外）。

#### 具材は「味とは別の軸」を持つ

味（調味料）は舌の感覚、具材は体の要求。軸が重ならないので混同しない。

| 具の軸 | 求めているもの | 具材例 |
|--------|--------------|--------|
| `FILLING`／腹持ち | 空腹を長く抑えたい | 割れた餃子皮、米麺、パン団子（芋団子系は甘い文脈が強く却下） |
| `POWER`／力 | 疲労時に肉気や栄養が欲しい | モツ、くず肉団子、魚つみれ、血豆腐 |
| `GENTLE`／いたわり | 胃・喉・歯に負担をかけたくない | 豆腐、冬瓜、煮崩した葉物 |
| `BITE`／歯ごたえ | 噛みたい、食べた実感が欲しい | すじ肉、軟骨、きくらげ、蓮根、筍 |
| `TREAT`／ごちそう | 祝い、給料日、見栄、小さな贅沢 | 海老、大ぶりの肉、貝、包みワンタン |

- 各軸に「なぜそれが欲しいか」が紐づいているので、**会話に落とせる**
  （「歩き通しで腹が減った」→FILLING、「歯が悪くてな」→GENTLE）。味の軸と同じ構造。
- `TREAT` だけは体の要求ではなく**心の要求**。貧しい街だから成立する軸で、
  節目の日に良い一杯を出す、という物語を作れる。
- 注意：`GENTLE`（具・噛む負担）と `MELLOW`（味・口当たり）は近く見える。
  会話では「噛むのがつらい」「口当たりを優しく」のように書き分ける。

#### 判定：タグで基本評価、favorite でクリティカル

```
Customer
  wanted_tags : 求める性質（味の軸＋具の軸）
  favorite    : 好きな具材id（クリティカル用）

判定:
  1. 椀のtags と wanted_tags の一致数 → 普通 / 美味しい
  2. favorite が椀に入っていれば → さらに1段上げる（とても好み）
```

- 性質を合わせるのは**知識**（この客は疲れているから POWER）、
  favorite を当てるのは**その人を知っていること**。知り合いになって初めて出る報酬。
- この形なら軸を細かく増やさなくてよい。粗い軸＋個体の favorite で客ごとの差が出る。

#### 「全部入れれば当たる」問題は構造で解決済み

4枠時代の懸念（`まろやか＋辛い＋酸っぱい＋苦い` で全客に当たる）は、
単一の対策ではなく**構造の組み合わせ**で塞がっている：

- 味を全部入れると具が0 → 最低評価
- 3枠なので味は最大2つ → 5軸のうち2つしかカバーできず当てずっぽうが効かない
- 具材は別軸なので、味を増やすと具の評価を捨てることになる（トレードオフ）
- 味を複数重ねる場合は**成立する組み合わせだけ**を登録し、未登録は最低評価

#### その他の決定事項

- **レシピは縛りではなく指針**。メニューごとに決まっている具材があり、残りの枠は自由。
  レシピ通りでなくてもよい（在庫が無ければ選べないという制約はある）。
...
```
一致数 = 3枠の中身のtags と wanted_tags の一致数
		※鍋(soup)のtags は数えない
		※一致数は wanted_tags 側をループして数える
		  （具材を重複させても同じtagは1回しか数えられない）

  0個      → BAD（イマイチ）
  1個      → OK（普通）
  2個以上  → GOOD（美味しい）

favorite（客の好物・具材id そのもの）が椀に入っていれば1段上げる
  ただし一致0個には効かない（BADのまま）
  1個＋favorite    → GOOD
  2個以上＋favorite → GREAT（とても好み）
```

- **鍋のtags を数えない理由**：鍋はその日の全客に共通で、特定の客への判断ではない。
  「今日の鍋がたまたま合った」という偶然を評価に混ぜない。
  将来 枠を4つ以上に増やすなら再検討する。
- **0個＋favorite がイマイチのままな理由**：合わない一杯に好物を入れられても嬉しくない。
  むしろ「もったいない」。favorite は基本を押さえた上のボーナスであって救済ではない。
- **favorite はレア食材にする**（たまにしか売っていない／高い）。
  情報を聞く → 市場で探す → 高い金を払って買う → その客が来たときに使う、という
  一連の行動になる。儲けは減るが喜ばせたい、という金と関係のトレードオフが生まれる。

#### メニュー（最小5種）

味1つ＋具1つが決まっており、残り1枠は自由。**メニュー通りでなくてもよい**
（例：ナムプリックパオ＋豆腐＝辛いが噛む負担の少ない一杯。メニューには無いが客には合う）。
メニューから外れるときは主人公の独り言（「メニューにはないが……」）で自覚させる。
メニューは安全策であり、崩すのは賭け。

| 味 | メニュー名 | 構成 |
|----|-----------|------|
| `HOT` | 火吹きモツ湯 | ナムプリックパオ、下処理したモツ |
| `MELLOW` | 白椰子豆腐湯 | ココナッツミルク、豆腐 |
| `SOUR` | 塩ライム冬瓜湯 | 塩漬けライム、冬瓜 |
| `BITTER` | 苦瓜モツ湯 | 苦瓜、モツ |
| `SAVORY` | 薬膳魚醤肉団子湯 | 薬膳ナンプラーだれ、くず肉団子 |

#### Day1 の在庫（最小・残りは後日買い足す）

Day1 は説明の日なので、その日の3人に応えられる分だけ持たせる。
塩漬けライム・苦瓜・冬瓜は無いので SOUR / BITTER のメニューは作れない。
2日目以降に市場で買い足すと、作れるメニューが増える＝**在庫が選択肢を決める**。

| 種別 | 具材 | 軸 |
|------|------|-----|
| 調味料 | ナムプリックパオ | `HOT` |
| 調味料 | ココナッツミルク | `MELLOW` |
| 調味料 | 薬膳ナンプラーだれ | `SAVORY` |
| 具材 | 下処理したモツ | `POWER` |
| 具材 | くず肉団子 | `POWER` |
| 具材 | 豆腐 | `GENTLE` |
| 具材 | 割れた餃子皮 | `FILLING` |

「くず肉団子」「割れた餃子皮」のように、**売り物にならないものを安く使う**方向で
具材を選ぶと、屋台の貧しさが具材そのものに出る。
...
**未定（実物を触ってから決める）**：味を2つ重ねるときの成立表 / 差し替え操作の形 /
メニューを増やすタイミング。
