...
→ 結論：**Graybox の前に調理の中身（具材の種類・レシピ・4枠の選択）を作る必要がある**。
DebugPanel が主（検証装置）、Graybox は仮（レイアウトと操作感の確認用）という役割は維持する。

### STEP 17.5：調理システム（メニュー＋4枠）を作る ← Graybox の前提

Graybox で操作感を測るには、まず「選ぶ対象」が要る。以下は今回の設計議論で決まった形。
...
- **椀に足せる上限は4つ**。選択肢はもっとある（10種類程度を想定）。
- **レシピは縛りではなく指針**。メニューごとに「決まっている具材」が 1〜4 個あり、
  残りの枠は自由。レシピ通りでなくてもよい（在庫が無ければ選べないという制約はある）。
- **注文と違うものを出しても、好みに合っていれば「美味しい」**。評価軸は満足度ひとつ。
  「注文と一致したか」は評価に含めない。レシピ通りが安全、外すのは賭け。
- **レシピは閲覧できるようにする**（メニューが増えても記憶ゲームにしないため）。
  メニュー定義は表示と調理の両方が同じデータを参照する。
- 選び方は「選択肢が並び、押すと椀に足される（4枠まで自由に）」。
  ADJUST で止まっている間は何度でも押せ、「提供」で進む。
- **取り消しは作らない。代わりに「廃棄」ができる**。作った椀を捨てて作り直す。
  取り消し（無コストの訂正）ではなく、失敗を引き受ける選択にする。
...
- 「まずい」は**今は作らない**。条件（嫌いな tag／同じ tag ばかり等）が未定で、
  滅多に出ないものを先に作っても検証できないため。実物を触って必要と分かってから。

**未定（実物を触ってから決める）**：評価の段階と閾値 / 具材と tags の種類 /
メニュー数 / 差し替え操作の形 / 「まずい」の条件。
