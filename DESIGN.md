...
料理の出来（GOOD/GREAT）→ 評判が上がる → 翌日のモブ客が増える
  → 鍋の残量が減る → 水を足す（かさ増し）→ 薄まる → 味が落ちる
  → ベースを足して戻す（金がかかる）
```

**成功が次の困難を生む**構造。制約は2つ：**足せる水は有限**（朝に汲んだ量が上限）、
**ベースは金がかかる**。この2つがあるから「客が増える＝単純に得」にならない。

### 時間経過：時間帯で区切る

実時間でも「客をさばくごと」でもなく、**営業時間を3つに区切る**。
会話をじっくり読めて、急かされない。客が来ない日でも時間は進む。

| 時間帯 | 内容 |
|--------|------|
| 宵の口 | 名前あり客1人 ＋ モブ客（総数の3割） |
| 夜半 | 名前あり客1人 ＋ モブ客（総数の5割） |
| 明け方 | 名前あり客1人 ＋ モブ客（総数の2割） |

- **夜半が山場**。一番混み、鍋が一番減る。判断のピークがここに来る。
- 名前あり客は毎日3人で固定＝**物語の量は変わらない**。増えるのはモブだけなので
  シナリオを書く負担が増えない。モブは鍋を減らす経営側の存在。
- 時間帯ごとに客層を変える表現もできる（宵の口＝仕事帰り、夜半＝夜勤明け 等）。

### 残量と濃さの減り方

| 数値 | 減る/増える理由 |
|------|----------------|
| 残量 | 客に出すと減る（1杯分）＋ 時間帯が変わると一定量減る（蒸発） |
| 濃さ | 時間帯が変わると上がる（煮詰まる） |

- 残量は**杯数で持つ**（「あと3杯」。残りの客数と比べられる）。
- 時間経過は「量が減って濃くなる」＝同じ現象（水分が飛ぶ）の2側面。
  水を足せば両方が同時に戻る（量が増えて薄まる）。
- 客に出していなくても鍋は減る＝閑古鳥の日でも鍋は傷む。

### モブ客の数

1日の総数を**評判で決め**、時間帯に 3:5:2 で配分する。
評判が上がるほど総数が増え、鍋の配分が厳しくなる。

### 鍋を触るタイミング

- **Day1 は強制イベント**。10人分仕込み、宵の口で足りなくなる。
  「水を足す → 薄まる → ベースを足す」を、困った状況で教える（必要になってから教える）。
  PREP でベースを2つ買っておき、1つを予備にする（後で役に立つ伏線）。
  閉店時に少し余る＝初日から失敗させない。
- **Day2 以降は いつでも触れる**。接客中でも鍋のボタンから水・ベースを足せる。
  鍋の状態が常に見えているから「そろそろ足さないと」と気づける。
...
- 屋台の主人から見た景色の再現。目の前に客、手元に鍋。
  注文を受けたら椀を用意し、鍋から取り分けて具を入れる。
- **残量はゲージと数字の両方**。ゲージで「減ってきた」と気づき、数字で
  「あと3杯、夜半は5人来る」と判断する。椀の画面に切り替わっても残量は見える。

### 未確定

時間で減る量の具体値 / 濃さの段階数と評価への影響 / 評判からモブ総数を出す式 /
時間帯の途中で鍋が尽きたときの扱い（客を断る？ 薄いまま出す？）
